...
export const ClinicAuditLogsPage: React.FC = () => {
  ...
  const fetchLogs = async () => {
    try {
      setLoading(true);
      const res = await apiRequest<{ logs: AuditLog[] }>('/api/clinic/audit-logs');
      setLogs(res.logs);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-2 border-b border-gray-200">
        <div>
          <h1 className="text-xl font-bold text-[#0A0A0A] tracking-tight">Clinic Audit Trail & Security</h1>
          <p className="text-xs text-gray-500 mt-0.5">
            Immutable log of appointment creations, staff changes, cancellations, and doctor schedules within this clinic.
          </p>
        </div>
        <Button
          variant="outline"
