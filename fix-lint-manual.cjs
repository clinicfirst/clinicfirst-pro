...
auth = auth.replace("password_hash: hashPassword(String(newPassword).trim()),\n      }", "password_hash: hashPassword(String(newPassword).trim()),\n      } as any");
fs.writeFileSync('server/routes/auth.routes.ts', auth);

// 2. clinic.routes.ts (563)
let clinic = fs.readFileSync('server/routes/clinic.routes.ts', 'utf8');
clinic = clinic.replace("const enrichedAppointments = appointments.map((apt) => {", "const enrichedAppointments = await Promise.all(appointments.map(async (apt) => {");
clinic = clinic.replace("          service_duration: service?.duration_minutes || 30,\n        };\n      });", "          service_duration: service?.duration_minutes || 30,\n        };\n      }));");
clinic = clinic.replace("const appointments = await AppointmentService.list(clinicId).filter", "const appointments = (await AppointmentService.list(clinicId)).filter");
