...
async function seedData() {
  console.log("Starting final data migration to Supabase...");

  // Sanitize users
  if (data.users) {
    data.users = data.users.map(u => {
      const { doctor_id, permissions, ...rest } = u;
      return rest;
    });
  }
  ...
  if (data.calls) {
    data.calls = data.calls.map(c => {
      const { active_ai_config_version, end_time, ...rest } = c;
      return rest;
    });
  }

  const tables = ['users', 'calls'];
